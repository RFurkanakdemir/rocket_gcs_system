...
void initDataPaket( GcsPaket_t * const pkt , HYI_t * const ptHYI , const uint8_t WHICH_NODE )
{
  ...
  memset( ptHYI->u8_array , 0 , sizeof ( HYI_t ) );
...
void veriPaketle ( HYI_t * const pkt , const uint8_t * const data )
{
  switch ( data[ 4 ] )
  {
    case ROCKET_NODE:
      {
        roketDataPaket_t * roketData = (roketDataPaket_t *) data;
        memcpy( &( pkt->rocket_node ) , &( roketData->data ) , sizeof( Roket_t ) );
      }
      break;
    case ROCKET_NODE_2:
      {
        roketDataPaket_t * roketData = (roketDataPaket_t *) data;
        memcpy( &( pkt->rocket_node_2 ), &( roketData->data ) , sizeof( Roket_t ) );
      }
      break;
      /*
    case PAYLOAD_NODE:
